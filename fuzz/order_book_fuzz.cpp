...
class ReferenceOrderBook final {
  public:
    ...
    void clear_side(bmd::BookSide side) {
        (side == bmd::BookSide::Bid ? bids_ : asks_).clear();
    }
  ...
  private:
    ...
    [[nodiscard]] LevelVec::iterator find_level(LevelVec& v, bmd::PriceUnits p) {
        for (auto it = v.begin(); it != v.end(); ++it) {
            if (it->price == p) return it;
        }
        return v.end();
    }

    [[nodiscard]] LevelVec::const_iterator find_level(const LevelVec& v,
                                                       bmd::PriceUnits p) const {
        for (auto it = v.begin(); it != v.end(); ++it) {
            if (it->price == p) return it;
        }
        return v.end();
    ...
    void sort_side(bmd::BookSide side) {
        ...
        if (side == bmd::BookSide::Bid) {
            std::sort(levels.begin(), levels.end(), [](auto& a, auto& b) { return b.price < a.price; });
        } else {
            std::sort(levels.begin(), levels.end(), [](auto& a, auto& b) { return a.price < b.price; });
        }
    }
...
void verify_no_zero_quantities(const std::vector<bmd::BookLevel>& levels) {
    for (const auto& l : levels) {
        if (l.quantity.value() <= 0) std::abort();
    }
}
...
void verify_strict_ordering(const std::vector<bmd::BookLevel>& levels, bool ascending) {
    for (std::size_t i = 1; i < levels.size(); ++i) {
        if (ascending) {
            if (levels[i - 1].price.value() >= levels[i].price.value()) std::abort();
        } else {
            if (levels[i - 1].price.value() <= levels[i].price.value()) std::abort();
        }
    }
...
void verify_no_duplicates(const std::vector<bmd::BookLevel>& levels) {
    for (std::size_t i = 1; i < levels.size(); ++i) {
        if (levels[i - 1].price == levels[i].price) std::abort();
    }
}
...
void check_consistency(const bmd::OrderBook& book, const ReferenceOrderBook& ref) {
    for (auto side : {bmd::BookSide::Bid, bmd::BookSide::Ask}) {
        if (book.level_count(side) != ref.level_count(side)) std::abort();
        const auto prod = book.all_levels(side);
        const auto refv = ref.all_levels(side);
        if (prod.size() != refv.size()) std::abort();
        for (std::size_t i = 0; i < prod.size(); ++i) {
            if (!(prod[i].price == refv[i].price)) std::abort();
            if (!(prod[i].quantity == refv[i].quantity)) std::abort();
        }
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    ...
    const auto max_ops = static_cast<std::size_t>(data[2]) | (static_cast<std::size_t>(data[3]) << 8);
    ...
    if (!ps.has_value() || !qs.has_value()) return 0;
    ...
    for (std::size_t i = 0; i < op_count; ++i) {
        ...
        const auto price_raw = static_cast<std::int64_t>(
            (static_cast<std::int64_t>(data[offset + 1] % 128) + 1));
        ...
        case 3:
            for (const auto& level : ref.all_levels(side)) {
                auto existing_q = bmd::QuantityUnits::create(
                    level.quantity.value());
                if (existing_q.has_value()) {
                    static_cast<void>(book.apply_level(side, level.price,
                                                        *existing_q));
                }
            }
            break;
        ...
        case 8: {
            ...
            for (std::size_t j = 0; j < 4 && offset + 2 < size; ++j) {
                ...
                if (rp.has_value() && rq.has_value()) {
                    if (j % 2 == 0) new_bids.push_back({*rp, *rq});
                    else new_asks.push_back({*rp, *rq});
                }
            }
            book.replace_all(new_bids, new_asks);
            ref.clear();
            for (const auto& l : new_bids) ref.apply_level(bmd::BookSide::Bid, l.price, l.quantity);
            for (const auto& l : new_asks) ref.apply_level(bmd::BookSide::Ask, l.price, l.quantity);
            break;
        }
        case 9:
        case 10:
