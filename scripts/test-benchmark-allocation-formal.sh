...
BMD_PROJECTION_BUILD_PROTO_ADAPTER:BOOL=ON
...
EOF
    case "$mutate" in
        ...
        coverage)
            ...
            ;;
        lto-on)
            sed -i '' 's/^CMAKE_INTERPROCEDURAL_OPTIMIZATION:BOOL=OFF$/CMAKE_INTERPROCEDURAL_OPTIMIZATION:BOOL=ON/' \
                "$dir/CMakeCache.txt"
            ;;
        lto-missing)
        ...
        no-benchmarks)
            ...
            ;;
        no-adapter)
            sed -i '' 's/^BMD_PROJECTION_BUILD_PROTO_ADAPTER:BOOL=ON$/BMD_PROJECTION_BUILD_PROTO_ADAPTER:BOOL=OFF/' \
                "$dir/CMakeCache.txt"
            ;;
        tests-on)
