...
Order Book Core — COMPLETE, M3 Sequence and Projection State — IMPLEMENTATION IN EXTERNAL CODE
REVIEW**. The library exposes stable project/version metadata, exact numeric primitives, a
deterministic order book core, and the review-stage M3 sequence-aware projection API.
...
> deterministic formatting. M2 subsequently added the deterministic order book core. M3 now adds
> review-stage implementation candidates for sequence validation and projection lifecycle state.
> Market-state snapshots, protobuf adapters, networking, persistence, and trading remain
> unimplemented.
...
The review-stage M3 API provides strongly typed `UpdateId` and valid-by-construction `UpdateRange`
...
[ADR-0005](docs/adr/ADR-0005-market-specific-sequence-policy.md). M3 implementation is in external
code review and is not yet complete.
...
Sequence and Projection State design is approved and its implementation is in external code review.
The Contracts reference baseline is
...
- The repository now includes numeric primitives, a deterministic L2 market-by-price order book,
  and the review-stage M3 sequence/projection implementation. M3 is not complete until external
  code review and all CI gates pass.
