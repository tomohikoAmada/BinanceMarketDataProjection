...
| R3 | `tests/projection_state/reference_projection.hpp` — REUSED in place with corrected Spot bootstrap semantics (own decision tables; Spot `U <= L + 1` bootstrap successor coverage per ADR-0008; live successor coverage; USD-M `U <= L <= u` then `pu` continuity). |
